...
- **(A) 구문분석** — 영어 문장을 주고 구조를 표시(끊어읽기/성분 표시)한 뒤 해석하게 합니다. (`독해원리` 스타일)
- **(B) 영작** — 같은 문법 포인트를 응용한 한국어 문장을 주고 영어로 옮기게 합니다. (`작문원리` 스타일)

:::info[⚠️ 중요한 비대칭]

ch01~04(기초 문법)는 아직 관계사·분사·접속사 같은 "문장을 길게 늘이는 도구"를 배우기 전이라, 이 구간의 "구문분석"은 긴 문장을 끊어 읽는 훈련이 아니라 **주어·서술어·목적어·보어를 정확히 식별하고 시제·수를 정확히 맞추는 훈련**입니다. ch05(수동태)부터 문장이 점점 길어지고, ch07(to부정사)~ch09(분사)를 지나면서 `독해원리`의 "종합 예문"과 같은 다중 수식어가 겹친 긴 문장이 등장합니다. 즉 이 문서는 챕터가 진행될수록 문장 길이와 밀도가 함께 늘어나도록 설계되어 있습니다.
...
**함께 보면 좋은 원리 문서**: [독해원리](/docs/reading-strategy)(긴 문장을 끊어 읽는 법) · [작문원리](/docs/writing-strategy)(문장을 만드는 법) · [조어원리](/docs/word-formation)(접사로 품사 바꾸는 법) · [동사원리](/docs/verb-principle)(동사가 문장구조를 결정하는 원리)
...
- 반복해서 틀리는 패턴이 있다면 [용어사전](/docs/glossary)에서 해당 용어를 다시 확인하고, ch01의 관련 Unit으로 돌아가 예문을 다시 읽으세요.

## 이 문서와 ch01~18의 관계

이 문서는 ch01~16의 문법 지식을 전제로 합니다. 각 챕터 섹션의 문제는 그 챕터의 Unit뿐 아니라 그 이전 챕터에서 배운 문법도 함께 사용합니다(누적 방식). 원리 자체를 다시 설명하지 않으므로, 막히는 문제가 있으면 원 챕터로 돌아가 "핵심 개념 설명"을 먼저 확인하세요.

## 참고

이 문서의 예문·해설은 전부 새로 작성한 오리지널 콘텐츠입니다. 다만 "챕터가 진행될수록 누적된 문법을 하나의 문장에 녹여 반복 훈련한다"는 문제집 형식 자체는 YouTube 채널 grammarTV(중학영어TV)의 "필수 구문 1000제" 강의 시리즈와 흔한 "패턴 영어 1000문장" 류 교재에서 아이디어를 얻었습니다.
