# 구문실전연습 · Chapter 01 — 문장의 기본 구조와 품사 (Unit 001~009)
...
---
### 챕터 종합 문제 (Unit 001~009 통합)

지금까지는 유닛 하나씩 따로 연습했습니다. 실제 문장에서는 여러 유닛의 규칙이 동시에 적용되므로, 아래 문제들로 "따로는 아는데 섞이면 못 푸는" 상태인지 확인합니다. 각 문제 앞에 관련된 Unit 번호를 표시했습니다.

**(A) 구문분석**

1. (Unit 002-1, 008, 009) Doesn't the new schedule affect everyone in the department?
   - 구조: Doesn't(does + not, 의문문 도치) / the new schedule(S, 3인칭 단수) / affect(동사원형 — does가 3인칭 단수 정보를 이미 가져감) / everyone in the department(O, 부정대명사+전치사구)
   - 해석: 새 일정이 부서의 모두에게 영향을 미치지 않나요?
   - 포인트: schedule이 3인칭 단수라서 does가 필요했지만(Unit 008·009), 본동사 affect는 원형 그대로입니다. everyone은 뜻은 여러 명이지만 문법적으로는 단수입니다(Unit 002-1).

2. (Unit 003, 004, 006) There isn't enough evidence to support these claims yet.
   - 구조: There isn't(단수 be동사 부정) / enough evidence(불가산명사, 진짜 주어) / to support these claims(수식어) / yet(부사)
   - 해석: 아직 이 주장들을 뒷받침할 충분한 증거가 없다.
   - 포인트: evidence는 항상 단수 취급하는 불가산명사이므로(Unit 004) isn't가 오고, There is/are 구문에서 진짜 주어는 There가 아니라 뒤의 명사라는 원칙(Unit 003·006)이 여기서도 그대로 적용됩니다.

3. (Unit 005, 006, 007, 008) Neither of us was ready, but she still finished the report on time.
   - 구조: Neither of us(단수 취급, S) / was(be동사 과거, 단수, Unit 006·007) / ready(형용사, 보어) / but / she(인칭대명사, Unit 005) / still finished(V, 과거, Unit 008) / the report(O) / on time(부사구)
   - 해석: 우리 둘 중 누구도 준비되지 않았지만, 그녀는 그래도 제시간에 보고서를 끝냈다.
   - 포인트: neither는 Unit 006에서 배운 대로 단수 취급하므로 was를 씁니다. 뒤 절은 과거형(finished)이라 3인칭 단수 -s 규칙(Unit 008)이 애초에 적용되지 않는다는 것도 함께 확인하세요(그 규칙은 현재형에서만 작동합니다).

**(B) 영작**

1. (Unit 002-2, 003, 006) 그 결과는 놀랍지 않았지만 매우 유용했다. (result, surprising, useful)
   > 모범답안: The result was not surprising, but it was very useful.
   - 자주 하는 실수: surprising과 useful은 둘 다 형용사로 be동사 뒤 보어 자리에 옵니다. 두 번째 절에서 주어를 반복하지 않고 대명사 it으로 받는 것이 자연스럽습니다(Unit 005와 연결).

2. (Unit 004, 009) 그 팀은 충분한 정보를 갖고 있지 않았나요? (team, enough information, have-had)
   > 모범답안: Didn't the team have enough information?
   - 자주 하는 실수: information은 불가산명사라서 "informations"라고 복수형을 만들 수 없습니다(Unit 004). 또한 did(didn't) 뒤의 본동사는 원형(have)이어야 하며 has처럼 다시 3인칭 단수형을 쓰면 틀립니다(Unit 009).
